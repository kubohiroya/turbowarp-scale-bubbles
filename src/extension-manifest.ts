...
function normalizeBlock(
  ...
  const opcode = requireNonEmptyString(
    block.opcode,
    `Block at index ${index} opcode`,
  );
  const blockType = requireNonEmptyString(
    block.blockType,
    `Block ${opcode} blockType`,
  );
  const argumentRecord = requireRecord(
    block.arguments ?? {},
    `Block ${opcode} arguments`,
  );
  const argumentsList = Object.entries(argumentRecord).map(
    ([argumentId, argument]) => {
      requireNonEmptyString(argumentId, `Block ${opcode} argument ID`);
      const definition = requireRecord(
        argument,
        `Block ${opcode} argument ${argumentId}`,
      );
      ...
      if (
        menu !== undefined &&
        (typeof menu !== "string" || !menuIds.has(menu))
      ) {
        throw new TypeError(
          `Block ${opcode} argument ${argumentId} references unknown menu: ${menu}`,
        );
      }
      return menu === undefined
        ? { id: argumentId, type }
        : { id: argumentId, type, menu };
    },
  );
  ...
  return {
    opcode,
    blockType,
    arguments: argumentsList.sort((left, right) =>
